...
class ProductProvider with ChangeNotifier {
  ...
  List<ProductModel> getNameProduct(String catName) {
    List<ProductModel> getProduct = _products
  ...
  final prdoductDB = FirebaseFirestore.instance.collection('products');

  List<ProductModel> getSearch(String querySearch, List<ProductModel> productsSearch ) {
    List<ProductModel> getProduct = productsSearch
  ...
  Future<List<ProductModel>> fetchDatat() async {
    try {
      await prdoductDB.get().then((productsSnapShot) {
        _products.clear();
        for (var elements in productsSnapShot.docs) {
          _products.insert(0, ProductModel.fromFireStore(elements));
        }
      });
      notifyListeners();
      return _products;
  ...
  Stream<List<ProductModel>> streamProduct() {
    try {
      return prdoductDB.snapshots().map((event) {
        _products.clear();
        for (var elements in event.docs) {
          _products.insert(
            0,
            ProductModel.fromFireStore(elements),
          );
        }
        return _products;
      });
